...
var hero_containers = []
var enemy = []
var enemy_spwans = [Vector3(0,0,0.5),Vector3(-2,0,0.5),Vector3(2,0,0.5)]
...
func _ready():
	...
	for path in State.encounter:
		if path != null:
			entity_container = preload("res://scenes/enemy_3d.tscn")
			instance = entity_container.instance()
			$enemies.add_child(instance)
			instance.transform.origin = enemy_spwans[e]
			e+=1
			...
			last_child.get_node("name").text = instance.name
			set_hp(last_child.get_node("healthbar/Viewport/HP"), instance.current_hp, instance.max_hp)
			last_child.get_node("sprite2").texture = instance.texture
...
func attack(attacker,target,skill,who):
	...
	if who == "ally":
		...
		set_hp(enemy_containers[index].get_node("healthbar/Viewport/HP"), target.current_hp, target.max_hp)
		yield(get_tree().create_timer(0.5), "timeout")
